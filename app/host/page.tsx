...
import { Target, Rocket, Plus, Trash2 } from "lucide-react";
...
export default function HostPage() {
    ...
    const handleCreate = async () => {
        ...
        try {
            ...
            router.push(`/host/${game.editCode}`);
        } catch (e) {
            setError(String(e));
            setLoading(false);
        }
        ...
        <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column" }}>
            ...
            <div className="flex bg-white/5 border-b border-white/10 p-4 items-center justify-between sticky top-0 z-10 backdrop-blur-md">
                ...
                <div className="flex items-center gap-4">
                    <button className="btn-ghost flex items-center gap-2 py-1.5 px-3" onClick={() => router.push("/")}>Exit</button>
                    <button className="btn-primary flex items-center gap-2 py-1.5 px-4" onClick={handleCreate} disabled={loading}>
                        {loading ? "Saving..." : <><Rocket size={16} /> Create & Save</>}
                    </button>
                </div>
            </div>
